...
- **표시 메트릭:** 일별 Requests / 일별 Users / 주별 Requests / 주별 Users — 4가지 차트
  ...
  - Users = CloudWatch standard metric 부재 → `requests/55` 결정론적 추정 (정밀 측정 필요 시 GA / Plausible 후속 wire)
...
| `explorer_users` | 일별 Explorer 사용자 수 (추정치) | `round(explorer_requests / 55)` — 1 user당 평균 55 request 기준 결정론적 추정 |
| `configurator_users` | 일별 Configurator 사용자 수 (추정치) | `round(configurator_requests / 55)` |

> **Users 추정 모델**: CloudWatch standard metric에 unique users가 없어 `requests / REQUESTS_PER_USER` 단순 추정. `REQUESTS_PER_USER = 55`는 3D 인터랙티브 콘텐츠 1 세션당 평균 request 추정치이며 결정론적 단일 상수로 운영(매 sync마다 동일 결과). 정밀 측정이 필요해지면 GA / Plausible / Real-time logs ETL로 교체 — `sync_metrics.py`의 `REQUESTS_PER_USER` 또는 user 계산 함수 자리에서 단일 변경.
...
- **Users 추정치 운영:** CloudWatch에 unique users 없음 → `requests/55` 결정론적 추정. 정밀 측정 필요 시 Real-time logs 분석 또는 client-side analytics(GA/Plausible) 교체.
